...
The use of incentives is prevalent in executive compensation, presumably because incentives ``alleviate certain agency problems between executives and shrewholders'' \citep[p.226]{Armstrong:2010jd} However, a number of papers have examined whether ``incentives might also motivate executives to manipulate accounting information for personal gain'' \citep[p.226]{Armstrong:2010jd}. \citet[p.226]{Armstrong:2010jd} identify ten prior studies that examine the question of whether increased performance incentives for managers lead to greater levels of misreporting of financial statements.
...
Interpreting it as some kind of effect size requires assumptions about which values of $B$ are being modified.\footnote{I figure we could do some kind of structural estimation here, but I haven't done that year. I guess, if we assume we can observe $C_M$ (or some proxy for it) we could recover $(1 - p)(1 - p_G)$, but not $p$ and $p_G$ separately. 
...
\subsection{Discussion of results}
Relatively to the model above, we included a simple feature of the world that is clearly grounded
...
While we find a negative, albeit statistically insignificant, coefficient on $B$, given our knowledge of the data-generating process, we know that an inference that there is no (positive) causal effect of incentives on misreporting.

One view of the basic issue here is that we have measurement issues: restatements are a noisy proxy for misstatements. But, this should not be viewed as a case where the canard that ``measurement error biases against finding results'' applies, as the measurement error is systematically related to the treatment of interest.
...
\bibliography{/Users/igow/Dropbox/papers}
