...
Nevertheless, even in this relatively straightforward setting, \citet{Michels:2015aa} recognizes the possibility of different materiality criteria for disclosed and recognized events, which could affect the relation been underlying events and observed disclosures, and takes care to address this concern.\footnote{The setting of \citet{Michels:2015aa} plausibly involves a natural experiment, the endogenous nature of the disclosure and reporting responses by firms to these events, which is what is observable to the researcher, makes drawing causal inferences about the effect of recognition versus disclosure problematic.}
...
First, it is a highly parametric assumption: parallel trends might hold in levels of a variable, they would not likely to hold if variables were log-transformed.
...
The causal interpretation of fixed-effect estimators is often unclear.
Consider a setting where there are three unobservable types: type $A$ receives a salary bump of \$20,000 from getting a degree and thus gets a degree by the time she is 21; type $B$ receives a salary bump of \$4,000 from getting a degree and thus waits till she is 26 before getting a degree; type $C$ never gets a degree (perhaps because the benefits are too small). 
Suppose a researcher has a panel of salary observations for $n$ individuals for each year between ages 21 and 30.
What causal effect of a degree will be estimated using a fixed-effects estimator?
It is easy to see that types $A$ and $C$ will not contribute to the estimate, as these types either always or never have a degree.
It is also straightforward to show that the estimated effect for the remaining type will be approximately \$1,400, which is not interpretable as an estimate of anything of interest.
It's not the effect of treatment on the treated, which would be some combination of \$4,000 and \$20,000.
It's not the effect of treatment on type $B$, which is \$4,000.

Yet this is a setting where a modicum of examination of the data would yield at least an estimate of the $4,000$ (simply regressing salary on an indicator for having a degree for those observations who wait till they are 26 before having a degree would yield this estimate).
While it's not clear that this ``treatment effect for those who wait to get a degree" would be of primary interest, it is also clear that the fixed-effect estimator does not produce anything of clear interest even in this simplistic setting.
...
Given the dominance of causal questions and observational data in accounting research, the very limited scope for quasi-experimental methods in accounting research may seem disappointing.
But we caution against wishful thinking regarding these methods as a basis for more than very small proportion of accounting research going forward. 
In the next section, we offer some alternative paths that accounting researchers might consider going forward.
...
A key implication of this discussion is that accounting researchers need to identify clearly and rigorously the causal mechanism that is producing the observed results in their research studies.
...
As a group, we do not have much experience assessing the likely contribution of descriptive research, and we are unfamiliar with many of the research methods that are used in descriptive research. 
However, given the possibility that descriptive research can help us identify actual causal mechanisms for accounting phenomenon, it would seem that accounting research would benefit from more high quality descriptive work in top journals.
