...
\item Increased use of structural models.
	...
	\item More descriptive research to actually understand the institutional phenomena of interest to accounting researchers.
...
% Peter's example:
% Consider what would happen to a presenter at a JAR conference who asserted that $\beta$ in the following regression of firm performance (e.g., ROE) on a corporate governance measure (e.g., the E index), 
%TODO: Relate this point to "structural model" discussion above.
% \[ ROE = E \beta + X\gamma + \epsilon ,\]
%represents the (causal) impact of governance on performance, We imagine the paper's discussant would utter the word endogeneity and that a heated exchange would ensue in which the phrases: "identification problem", "Wu-Hausman test", "over-identification test", "instruments", etc. were bandied about. Someone in the audience might even suggest reverse causality. Our point is no statistical model or test can resolve the underlying question of what causes what.

% How does the profession decide who is right then? 
% Even if we could agree that only one variable causes the other, simply writing down a linear model does not mean the two measures are linearly related. 
% Further, the validity of any instrumenting strategy or statistical tests would be predicated on the maintained linear functional form. 
% So how could one proceed given one only has observational data?

% Peter's text (but not his examplem [which is above]):
% Those who believe in structural models would argue that the advantage of writing down a firm decision-making model is it would allow others to understand what assumptions were made to establish a specific causal relation. Such models at the very least also would make clear conceptually the data experiments the researcher has in mind when they assert that in the observed data "$x$ causes $y$".
