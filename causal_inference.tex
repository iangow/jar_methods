...
Our survey of accounting research in 2014 identified five papers that exploited either a ``natural experiment'' or ``exogenous shock'' to identify causal effects \citep{Lo:2013jk,Aier:2014ii,Kirk:2014gx,Houston:2014hv,Hail:2014fq}.
An examination of these papers reveals how difficult it is to find a clear, natural experiment in observational data.

\cite{Aier:2014ii} exploit a 1991 Delaware court ruling that ``expanded the scope of directors' fiduciary duties to include creditors when a Delaware incorporated firm is in the `vicinity of insolvency'" as a ``natural experiment'' for the purpose of understanding the causal effect of debtholders' demand for conservatism (the treatment variable) on financial reporting conservatism (the outcome of interest).
...
In this case, \citet{Kirk:2014gx} would like to run an experiment whereby Reg FD was assigned to firms randomly, allowing them to compare the estimated effects of Reg FD on the two different groups.
...
Instead, \citet{Kirk:2014gx} might have more plausibly used an difference-in-difference design for their analysis, but would have needed to account for possible confounding given non-random assignment.
...
One paper with a credible natural experiment is that studied by \citet{Michels:2015aa}.
\citet{Michels:2015aa} exploits the difference in disclosure requirements for significant events that occur before financial statements are issued that differ according to whether the event occurs before or after the balance sheet date.
He finds evidence that the market reacts more strongly to recognized events.
Even with such a credible natural experiment, there are concerns that can arise.
As recognized by \citet{Michels:2015aa}, there are possibly different materiality criteria that affect the relation been underlying events and the disclosures the he relies on.

Another credible natural experiment is examined in \citet{Li:2015he}, who study the experiment whereby the SEC ``mandated temporary suspension of short-sale price tests for a set of randomly selected pilot stocks" \citet[p.\,80]{Li:2015he}.
...
Certainly researchers should exploit these natural experiments when they occur \citep[as in][]{Michels:2015aa}, but that care is needed in doing so and that researchers should not to stretch the concept to situations where it does not apply.
