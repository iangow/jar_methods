...
\begin{abstract}
	The vast majority of accounting research papers draw causal inferences from observational (or non-experimental) data, notwithstanding the well-known difficulties with doing so.
	This paper examines different approaches accounting researchers use to draw causal inferences. It is rarely the case that the data and
	methods used satisfy the assumptions required to draw causal inferences. We believe that accounting research would benefit from: a 
	greater focus on the study of causal mechanisms (or causal pathways); increased emphasis on structural modeling of the phenomena 
	of interest; and, more in-depth descriptive research. We argue these changes are possible and offer a practical path forward.
...
%Seems like we should move the thanks to the title page
\thanks{We thank Philip Berger, Eugene Soltes, Dan Taylor and Charlie Wang for helpful discussions.}
...
\begin{quote}\begin{singlespace} {\addtolength{\leftmargin}{.25in}\addtolength{\rightmargin}{.25in}
...
We found that overwhelmingly these papers used observational data and descriptive statistical models. For example, many believe that regression models can consistently estimate the causal impact of $X$ on $y$. This research makes strong assumptions about the functional relationship between $X$ and $y$, and that sufficient controls are present to account for changes in conditions related to changes in $X$.
Section \ref{sec:causal} examines these assumptions in detail. We introduce causal graphs, a device that can clearly communicate the causal reasoning underlying an empirical model. These graphs should prove useful to accounting researchers who wish to communicate the cause-and-effect logic underlying their analysis.
...
% Causal diagrams are of course no magic fix for the modeling issues raised by observational data.
...
% Do we want to say something about the amazingly small number of attempts to deal with endogeneity and causal inferences here?  Only about 15% of the papers.
...
\vglue -10pt
\begin{itemize}
\item At a minimum, there should be an increased emphasis on clearly communicating the assumptions needed to draw a causal inference. Here we believe that causal diagrams can help clarify what assumptions an accounting researcher must make about putative causes and putative effects. Researchers also need to be clearer about why these assumptions are reasonable given the specifics of their application.
...
\citep[e.g.][]{Cohen:2014jl,Clorproell:2014cv}. Abstracts and conclusions also reveal that authors see causal inferences as central. 
...
We recognize that some authors might disagree with our classifications. We know some would say their paper only claimed that ``theory predicts $X$ is associated $Y$ and, consistent with that theory, we show $X$ is associated with $Y$". However in the abstracts, introductions and conclusions of papers, these qualifications are rarely present. Instead the writing suggests the evidence in the paper tilts the scale in the direction of confirming a causal hypothesis.\footnote{
...
%TODO: Seems like we eventually want to add some type of Table to describe the literature survey and classification.
...
There is no way to do this without a theory, even if that theory only involves verbal reasoning about the causal connection. Statistics can help us determine whether $x$ and $y$ are (cor)-related, but correlations do not establish causality. The role of theory is to describe a conceptual experiment in which the researcher can unambiguously manipulate one variable independently of all other conditions that might explain changes in the other variable. These conceptual experiments may not be implementable in practice. However their value is that they provide a basis for understanding what assumptions are being made about the data generating process for the observed data.


\subsection{Causal diagrams: A primer}
Computer and decision scientists, among other disciplines, have recently sought to develop an analytical framework for thinking about causal models and their connection to probability statements \citep{Pearl:2009kh}.
Pearl's framework, which he calls the structural causal model, uses  causal diagram to describe causal relationships.
...
% A key tool in this approach is a causal diagram or causal graph. 
...
\citet{Pearl:2009vo} (see Appendix \ref{append}) shows that very different conditioning strategies are required for each of the causal diagrams. The boxes (or ``nodes") in the diagrams represent random variables and the arrows (or ``edges") connecting boxes represent hypothesized causal relations, with each arrow pointing from a cause to a variable assumed to be affected by it.\footnote{
...
\citet{Pearl:2009vo} shows that, if we are interested in assessing the causal effect of $X$ on $Y$, that we may be able to do so by conditioning on a set of variables, $Z$, that satisfies the ``back-door criterion" \citep[p.79]{Pearl:2009vo}.\footnote{A "back-door" path can be thought of as a way for $X$ to indirectly affect $Y$ by $X$'s association with other variables.  See Appendix \ref{append} for details.}
...
Figure \ref{fig:confound} depicts the conventional case where it is necessary to condition on $Z$ to estimate the causal effect of $X$ on $Y$.
...
Pearl  terms $Z$ in this case a \emph{confounder}.

Figure \ref{fig:mech} is a different. Here, $Z$, \emph{mediates} the effect of $X$ on $Y$. No conditioning is required in this setting to obtain an unbiased estimate of the effect of $X$ on $Y$.
The back-door criterion not only implies that we need not condition on $Z$ to obtain an unbiased estimate of the causal effect of $X$ on $Y$, but that we should not condition of $Z$ to get such an estimate. 
...
The back-door criterion not only implies that we need not condition on $Z$, but that we should not condition of $Z$ to get an unbiased estimate of the causal effect of $X$ on $Y$.
...
\cite{Aier:2014ii} exploit a 1991 Delaware court ruling that ``expanded the scope of directors' fiduciary duties to include creditors when a Delaware incorporated firm is in the `vicinity of insolvency'" as a ``natural experiment.'' They use this ruling to identify the causal effect of debtholders' demand for conservatism (the treatment variable) on financial reporting conservatism (the outcome of interest).
...
Given that the treatment is having an IR department, only a event that randomly assigned firms to having or not having such a department would qualify as a truly natural experiment. An additional interpretational issue is whether Reg FD impacted firm outcomes only through IR departments. To know this, we require a correctly specified model of firm outcomes. Otherwise, there is a risk that
a nonrandomly assigned Reg FD stands in for omitted variables.
...
But this is not what is needed for a valid natural experiment, which would instead randomly sort firms into those with political connections and those without. Without
such random assignment, one must lean heavily on having an otherwise correct model of loan costs and terms.
...
\begin{quotation}\begin{singlespace} {\addtolength{\leftmargin}{.25in}\addtolength{\rightmargin}{.25in}
...
As we discuss below, this type of research is essential for those who seek to develop structural models or improve our understanding of causal mechanisms.
%DL:  clean up the references in this footnote.
\footnote{
...
That private communication with management is an important source of information is confirmed by  \citet{Brown:2015kd}. \citet{Brown:2015kd} survey and interview financial analysts to understand how they think about a variety of issues. Their findings suggest that analysts' views on earnings quality differ from those researchers focus on. For instance, analysts do not use the ``red flags" used by academics to identify manipulation; and analysts generally are not attempting to uncover manipulation.
% Unintelligible sentence end:  and,  that forecasts are used to figure out the stock price target and are not an end in themselves.
