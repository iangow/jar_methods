...
Section \ref{sec:causal} examines these assumptions in detail. We introduce causal graphs, a device that can clearly communicate the causal reasoning underlying an empirical model. These graphs should prove useful to accounting researchers who wish to communicate the cause-and-effect logic underlying a regression analysis.
...
the criterion developed by \citep{Pearl:2009vo} (see Appendix \ref{append}) implies that very different conditioning strategies are needed for each of the causal diagrams. The boxes (or ``nodes") represent random variables and the arrows (or ``edges") connecting boxes represent hypothesized causal relations, with each arrow pointing from a cause to a variable assumed to be affected by it.\footnote{
...
Figure \ref{fig:mech} is a bit different, and here we see that $Z$, which is a \emph{mediator} of the effect of $X$ on $Y$. No conditioning is required in this setting to obtain an unbiased estimate of the effect of $X$ on $Y$.
