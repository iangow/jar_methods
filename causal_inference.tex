...
\begin{abstract}
	...
	The vast majority of accounting research papers draw causal inferences notwithstanding the well-known difficulties in doing so with observational data.
...
To provide insights into what is actually done in empirical accounting research, we examined all papers published in the leading accounting journals in 2014. 
We found that  the empirical papers overwhelmingly used observational data and descriptive statistical models to estimate the causal impact of "$X$ on $y$." 
...
We believe that these graphs are very useful for communicating the cause-and-effect logic underlying the typical regression analyses that rely on observational data.
...
In Section \ref{sec:quasi}, we examine and evaluate the use of quasi-experimental methods in accounting research. Quasi-experimental methods
produce credible estimates of causal effects only under very strong maintained assumptions about the model and the data relied upon. 
For example, variations in treatments are rarely random, the list of controls rarely exhaustive, and instruments do not always satisfy the
necessary inclusion and exclusion restrictions.
...
\item There should be an increased use of structural modeling methods. Structural models provide a more complete characterization of the behavior and institutions that produce a phenomenon of interest.
...
We believe that causal diagrams can be a useful tool to convey the key elements of a structural model and can also act as a middle-level stand-in when structural modeling of a phenomenon is in its early stages or is incomplete.\footnote{``Middle-level" here referring to the placement of causal diagrams between relatively informal verbal reasoning and the rigors of a structural model.}
...
Instead the writing suggests that the evidence in the paper tilts the interpretation in the direction of confirming a causal hypothesis.\footnote{
...
Work by \citet{Rubin:1974im,Rubin:1977dv} and \citet{Holland:1986p7458} formalizing ideas from the potential-outcome framework of \citet{Neyman:1923aa} have led to the so-called Rubin causal model. 
...
An important point worth emphasizing is that model-based causal reasoning is distinct from statistical reasoning. Suppose we observed data on $x$ and $y$ and 
make the strong assumption that we know that causality is one-way. How do we distinguish between whether $x$ causes $y$ or $y$ causes $x$? 
...
While conditioning onvariables is much like the standard notion of ``controlling for" such variables in a regression, there are critical differences.
First, conditioning means estimating effects for each distinct level of the set of variables in $Z$. This nonparametric concept of conditioning is not
what one does when one includes another regressor in a linear regression model. Second, the inclusion of a variable in $Z$ may not be an appropriate
conditioning strategy. Indeed, it can be that the inclusion of $Z$ results in biased estimates of causal effects.
...
While many of these variables should be considered confounders, less attention is given to explaining why they might not be mediators or colliders.
...
But exogeneity is not the same as saying it represents the effect of interest.
...
Most researchers would argue that we have all the ingredients for a successful treatment effect study. In particular assignment to treatment is not just
exogenous, but random. Now imagine that patients actually have to take the drug for it to have an effect. In this case, if there are unobserved reasons
why some assigned to treatment opt out, modify the dosage, or stop taking medications for which their might be interactions, then being assigned to
treatment is not the same as treatment. To take an extreme example, suppose the drug has a slight negative effect on blood pressure, everyone in fact takes the drug, 
but doctors in the hospital where patients are treated tell patients to stop taking their regular blood pressure medication. 
...
In sum, showing a treatment
is ``exogenous" does not guarantee that a regression would uncover the causal effect of interest. 
...
One such paper is \citet{Michels:2015aa}, who exploits the difference in disclosure requirements for significant events that occur before financial statements are issued. His events (e.g., fires and natural disasters) are plausibly exogenous and he finds evidence that the market reacts more strongly to those events that must be recognized. Nevertheless, even in this relatively straightforward world,  recognizes that there are possibly different materiality criteria that affect the relation been underlying events and disclosures. In his paper, he takes care to address this concern.

Another credible natural experiment is examined in \citet[p.\,80]{Li:2015he}, who study a regulatory experiment in which the SEC ``mandated temporary suspension of short-sale price tests for a set of randomly selected pilot stocks." \citet[p.\,79]{Li:2015he} conjecture ``that managers respond to a positive exogenous shock to short selling pressure \dots by reducing the precision of bad news forecasts." Although this regulatory change is about as good as it comes to having a
natural experiment by a regulator, it is important to recognize that this change could conceivably affect the properties of bad news forecasts.
...
Furthermore, there are good reasons to believe that the draft lottery does not affect anything else directly except for draft eligibility. These points
notwithstanding, even this study has been critiqued because the lottery instruments are weak.
...
Unfortunately, there are few (if any) accounting variables that meet the strong test that there are exogenous and not variables that directly
impact the outcome of interest. Consider, for example, instruments such as firm size, leverage, corporate governance, or director age. 
Not even director age can be taken as exogenous. Further, its hard to believe that variables such as firm size and leverage can be
excluded from the equation of interest. Sometimes researchers turn to lagged values of endogenous variables, industry averages or fixed
effects as instruments. But these too are subject to similar criticisms.\footnote{See \citet{Reiss:2007ej} for a discussion regarding the inability to claim that industry averages 
...
It is fairly common for accounting researchers to claim that statistical tests can resolve the question of whether their instrument is ``valid."
% American usage requires that periods and commas go in quotes. Doesn't make sense to me (I'm not American).
Indeed, many studies choose to test the validity of their instrumental variables using a statistical test \citep[see][]{Larcker:2010fq}.
...
The use of more tests or more sophisticated tests of instruments may not help. 
...
Assuming that $X$ and $\epsilon$ are bivariate-normally distributed with variance of $1$ and $\rho(X, \epsilon)=0.2$, N=100, and that $\sigma_{\eta}=0.03$, we 
performed 1000 IV regression simulations. Both the OLS and IV coefficients are close, with the IV estimated coefficient averaging $0.201$. The
IV coefficient estimates are statistically significant at the 5\% level 100\% of the time.\footnote{Note that this coefficient is close to $\rho(X, \epsilon) = 0.2$, which is to be expected given how the data were generated.} 
...
It is quite possible for completely spurious instruments to deliver bad inferences, yet easily pass tests for weak instruments and tests of overidentifying restrictions. Obviously, causal inferences based on such instrumental variables is completely inappropriate.
...
Unfortunately, this assumption seems inconsistent with the findings of \citet{Gow:2013aa}, who provide evidence that firms calibrate compensation plans (i.e., factors that directly affect $\textit{Comp}_{t+1}$) to comply with ISS's policies. (They do this to get a favorable $\textit{ISS recommendation}_t$, implying a path from $\textit{ISS recommendation}_t$ to $\textit{Comp}_{t+1}$ that does not pass through $\textit{Shareholder support}_{t}$.
Thus, this evidence suggests that the instrument of \citet[p.\,912]{Armstrong:2013io} is not credible.\footnote{\citet{Armstrong:2013io} recognize the possibility that the instrument they use is not valid and conduct sensitivity analysis to examine the robustness of their result to violation of the exclusion restriction assumptions. 
...
This is perhaps not unsurprising, as most studies do not have a theoretical model that can explain why a variable can naturally be excluded from
the equation of interest but still matter. Thus, why in theory instruments work, in practice accounting research has more work to do to justify
...
For example, whether an executive compensation plan is approved is a discontinuous function of shareholder support \citet{Armstrong:2013io} and whether a firm had to comply with provisions of the Sarbanes-Oxley Act is seen as a discontinuous function of market float \citep{Iliev:2010ic} .
...
For example, in designating a public float of \$75 million, the SEC may have reasoned that at that point the benefits of Sarbanes-Oxley, which may have been increasing in firm size, were approximately equal to the fixed costs of complying with the law.
If true, we would expect to see an estimate of approximately zero effect, even if there benefits of the law for shareholders of firms well above the threshold are substantial.
...
We encourage researchers using RD designs to employ methods that exist to estimate optimal bandwidths and the results estimates of causal effects \citep[e.g.,][]{Imbens:2011}.
...
However, we suggest caution. The assumptions required for them to deliver credible estimates of causal effects are unlikely to be met in many applications that rely on observational data. While this possibility should not stop their use, this does suggest that researchers might temper their conclusions based on the assumptions required to draw them. In particular, researchers should consider whether even if the necessary assumptions are met, how widely their conclusions are likely to generalize beyond the (special) environment they have found.
...
In the first half of the paper, we have argued that, while causal inference is the goal of most accounting research, it is difficult to find datasets and
statistical methods that can produce reliable estimates of causal effects. Does this mean accounting researchers must give up making causal statements? 
We believe the answer in no. There are viable paths forward. These paths do not rely on researchers finding clever and appropriate identification strategies to answer questions of interest. The objective of the second part of this paper is to discuss these paths forward.
...
in which they have used observational data to draw suggest the presence of causes and effects. Epidemiology and medicine are two fields that are often singled out in this regard. In what follows, we briefly provide examples and highlight the features of the examples that enhanced the credibility of the inferences drawn.
...
Later research provided ``compelling evidence that smoking causes oxidative modification of biologic components in humans.\footnote{This evidence of much higher levels of a new measure (levels of $F_2$-isoprostanes in blood samples) of the relevant oxidation in the body due to smoking.
This conclusion wass greatly strengthened by the finding that levels of $F_2$-isoprostanes in the smokers fell significantly after two weeks of abstinence from smoking" \citep[pp.\,1201--2]{Morrow:1995gz}.}
...
%We believe that the value of studying mechanisms is underestimated, perhaps due to a belief that the path to credible causal inferences involves clever identification strategies. However, even with such a strategy, we believe that causal mechanisms have an important role.
...
For example, evidence of trading activity by local investors while driving prior to, but not after, the implementation of distracted driving restrictions would seem to quite persuasive even incremental to the compelling identification strategy provided.\footnote{
...
In particular, we believe that structural models will see greater use in the coming years.
Finally we see great value to in-depth descriptive studies that inform 
causal issues and deepen our knowledge of the behavior and institutions we seek to model.
