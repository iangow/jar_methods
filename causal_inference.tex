...
\begin{abstract}\normalsize
	The vast majority of research papers draw causal inferences from observational (or non-experimental) data, notwithstanding the well-known difficulties with doing so.
...
This paper evaluates the different approaches accounting researchers use to draw causal inferences from observational data. Our discussion draws on developments in 
fields such as statistics, econometrics and epidemiology. Our goal is to identify areas for improvement and suggest how empirical accounting 
research can increase its impact.
...
For instance, many researchers who estimate so-called treatment effect models are aware that random assignment is often 
required to ensure that differences between the treatment and control samples is due to the treatment. Despite this awareness,
many accounting "treatments" are not randomly assigned, and the researcher then is in the position of arguing that they
can account for all other reasons that might lead to a difference between the treatment and control sample outcomes.

To understand what is actually done in empirical accounting research, we examined all empirical papers published in the leading accounting journals
in 2014. We found that overwhelmingly these papers drew causal inferences from statistical models, such as regressions. These studies drew their inferences assuming that they had included sufficient controls for changes in conditions related to the treatments of interest.
In Section \ref{sec:causal}, we examine statistical models commonly used to draw causal inferences.
To structure our discussion, we use causal diagrams (advocated by among other \citep{Pearl:2009kh}). These diagrams can help clarify the assumptions that are required to draw a causal inference from observational data. In particular, causal diagrams are useful means for representing the cause and effect logic of regression analyses, as they make it clear what variables are controls and which are not. 
...
Ultimately we believe that accounting research needs to lean less heavily on descriptive statistical models and observational data. Method is not a panacea for the problems that can arise in non-experimental data. In the second part of the paper we seek to identify approaches that provide a viable path forward for accounting research. 
...
However, by stating that ``consistent with \dots theory, $X$ is associated with $Y$," the clear purpose is to argue that the evidence tilts the scale, however slightly, in the direction of believing the theory is valid description of the real world: in other words, inference.\footnote{
...
If treatment assignment is driven by unobserved confounding variables, then DD and fixed-effect estimates will be biased and inconsistent. We are not confident that there are many settings in accounting that are likely to satisfy random treatment assignment. This then means that there is a (strong) burden of the users of these methods to explain why they believe that DD or fixed-effect estimators allow them to recover unbiased estimates of causal effects.
...
An important point worth emphasizing is that model-based causal reasoning is distinct from statistical reasoning. This point seems to be understood but regularly forgotten. Consider what would happen to a presenter at a JAR conference who asserted that $\beta$ in the following regression
of firm performance (e.g., ROE) on a corporate governance measure (e.g., the E index), 
%TODO: Relate this point to "structural model" discussion above.
\[ ROE = E \beta + X\gamma + \epsilon ,\]
represents the (causal) impact of governance on performance, We imagine the paper's discussant would utter the
word endogeneity and that a heated exchange would ensue in which the phrases: "identification problem", "Wu-Hausman test", "over-identification test", "instruments", etc. were bandied about. Someone in the audience might even suggest reverse causality. Our point is no statistical model or test can resolve the underlying question of what causes what.

How does the profession decide who is right then? 
Even if we could agree that only one variable causes the other, simply writing down a linear model does not mean the two measures are linearly 
related. Further, the validity of any instrumenting strategy or statistical tests would be predicated 
on the maintained linear functional form. So how could one proceed given one only has observational data? Those who believe in structural models would argue that the advantage of writing down a firm decision-making model is it would allow others to understand what assumptions were made to establish a specific causal relation. Such models at the very least also would make clear conceptually the data experiments the researcher has in mind when they assert that in the observed data "$x$ causes $y$".

To underscore this last point, imagine that a structural modeling exercise determined that $x$ and $y$ are linearly related, that $x$ causes $y$, but that $x$ and the unobserved error are correlated. If they were just interested in predicting $y$ with $x$ (a statistical problem), then OLS works just fine because it delivers the best linear unbiased predictor of $y$. On the other hand, it the modeler is interested in an experiment NOT observed in the data, which is what would be the best prediction of $y$ assuming that $\epsilon$ could be held fixed while $x$ changes, then OLS is not fine and can be improved upon. Notice that in this latter case the structural modeler is interested in a counterfactual world where $x$ can be randomized or manipulated independently of $\epsilon$.
