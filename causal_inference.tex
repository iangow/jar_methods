...
Since a theory inevitably involves causal relations, such inference is, we argue, inherently causal inference.\footnote{
Thus papers that seek to estimate a causal effect of $X$ on $Y$ are a subset of papers we classify as causal.
...
The more common approach we observe is the use of estimation approaches such as ordinary least-squares (OLS) regression, difference-in-difference estimates, and propensity-score matching for estimating coefficients that the authors use to draw causal inferences.
...
But, one treatment of interest (i.e., being VC-backed) is likely implemented well before the ``pre-treatment" period, making it difficult to consider the pre-treatment values of the outcome variable as not being caused by the treatment.
...
It is important to note that PSM does \emph{not} provide ``the closest archival approximation to a true random experiment and represents the most appropriate and rigorous research design for testing the effects of an ex ante treatment" \citep[p.\,1429]{Kirk:2014gx}.
...
Work by \citet{Rubin:1974im,Rubin:1977dv} and \citet{Holland:1986p7458} formalizes ideas from the potential-outcome framework of \citet{Neyman:1923aa} to develop the Rubin causal model. 
...
Figure \ref{fig:confound} is straightforward and it is likely intuitive for all accounting researchers that we need to condition on $Z$ and that, by doing so, we can estimate the causal effect of $X$ on $Y$.
...
%TODO: I think I need to give a more intuitive account of this example.
...
Of the 91 papers in accounting research in 2014 seeking to draw causal inference from observational data, we identify 14 that use quasi-experimental methods for inferences.
