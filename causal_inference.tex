...
\begin{titlepage}
  \centering
  	\begin{large}
  	\textbf{Causal Inference in Accounting Research\footnote{We thank seminar participants at London Business School, Karthik Balakrishnan, Philip Berger, Robert Kaplan, Alexander Ljungqvist, Eugene Soltes, Daniel Taylor, Ro Verrecchia, Charlie Wang, and Anastasia Zakolyukina for helpful discussions and feedback.}} \\	
  	\end{large}
  	\vspace{60pt}
	\textbf{Ian D. Gow} \\
...
why some assigned to treatment opt out, modify the dosage, or stop taking medications for which there might be interactions, then being assigned to
...
Unfortunately, there are few (if any) accounting variables that meet the strong test that they are exogenous, and not variables that directly impact the outcome of interest.
...
If true, we would expect to see an estimate of approximately zero effect, even if there were substantial benefits of the law for shareholders of firms sized well above the threshold.
