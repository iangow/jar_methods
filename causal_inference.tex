...
\begin{abstract}\normalsize
	...
	While some papers seek to use quasi-experimental methods to claim unbiased estimates of causal effects, we find that these methods are by accounting researchers inappropriately and thus do not support such claims.
	Furthermore, we argue that settings in which quasi-experimental methods could appropriately be applied are likely to be rare in accounting research.
	We believe that accounting research would benefit from greater focus on the study of causal mechanisms (or causal pathways), increased emphasis on structural modeling of the phenomena of interest, and more in-depth descriptive research.  
	% While our suggested approach does not completely resolve the  controversies associated with drawing causal inferences from observational data, 
	We argue that this provides a viable and practical path forward for accounting research.
\end{abstract}
...
Most empirical research in accounting relies primarily on observational data, i.e., data produced by observation of processes completely outside the control of the researcher.
The goal of this paper is to evaluate the approaches used by accounting researchers in making causal inferences using observational (or non-experimental) data and, drawing on developments in fields such as statistics and epidemiology, identify opportunities for improvement.
...
Merely documenting atheoretical correlations would provide no basis for predictions about what would happen in different circumstances, whereas using data to make inferences that support or refute broader theories does facilitate these kinds of predictions.
...
%TODO: Cite some classic reference here.
However, with observational data, treatment assignment is outside the control of the researcher and quite likely driven by forces that confound attempts to estimate causal effects using approaches that would appropriate is researcher were using experimental data.

From an examination of research published in accounting in 2014, we find that most papers seeking to draw causal inferences use traditional methods such as regression analysis with potential confounding variables included as controls.
Section \ref{sec:causal}, we examine the use of these traditional methods and the appropriateness of their use for causal inference.
To structure our discussion, we use the technology of causal diagrams \citep{Pearl:2009kh}.
We argue that causal diagrams help to clarify thinking about the causal relations assumed by the researcher and, using straightforward reasoning, provide clarity about the causal inferences that can (and cannot) be drawn from observational data.
With regard to regression approaches, causal diagrams provide guidance regarding such practical issues as which covariates should be included as controls in regressions and, an often overlooked issue, which should \emph{not} be included.
...
% This is probably consistent with Rubin's usage in effect.
...
We argue that more transparent reasoning, perhaps with the aid of causal diagrams, would have revealed the flaws apparent in these studies.
...
% Dave: "One especially promising path is use of field experiments with randomized treatments to measure causal effects (a good example -- Roberts QJE paper)." 
...
Ultimately we believe that accounting researchers need to shift their focus from the ``false promise" of causal inference using quasi-experimental methods.
These methods are generally misused in accounting research and, even if used correctly, have likely have very limited applicability in accounting research settings.
...
We identify emerging approaches in accounting research and also draw on other disciplines to offer a vision for how accounting research might successfully address causal questions even when clever instruments or natural experiments do not provide sharp, straightforward answers to the questions of interest to accounting researchers.
...
A more compelling path forward is threefold:
\begin{itemize}
\item Increased emphasis on the study of causal mechanisms. We argue that credible evidence of causal pathways between putative causes and putative effects can provide persuasive scientific evidence of causal effects even in the absence of appropriate experimental or quasi-experimental methods for direct estimates of those effects.
\item Increased use of structural models.
	A well-formulated causal diagram can be viewed as a non-parametric structural model, a structural model can be formulated in a more precise, parametric fashion and actually taken to data.
	%TODO: Flesh this out somewhat. What are the benefits of doing structural modeling?
	\item More descriptive research to understand the more deeply phenomena that we study.
	...
	More commonly, hypotheses are based on fairly heuristic reasoning drawing from prior research and intuitive guesswork.
	%TODO: The 2017 JAR call for papers is arguably evidence of this concern being shared by others. Considering adding a footnote saying this.
	A sounder basis for hypothesis development could be one drawing on detailed descriptive evidence of interesting phenomena.
	Additionally, an increased focus on descriptive studies is likely to help us to identify causal pathways, and help build better models of the phenomena of interest.
	%TODO: Trim the "descriptive research" bullet point.
\end{itemize}

The balance of the paper is structured as follows.
In \ref{sec:causal}, we provide an overview of causal inference in accounting and discuss key frameworks for examining causal issues.
In Section \ref{sec:quasi}, we examine and evaluate the use of quasi-experimental methods.
We then turn of outlining a path forward for accounting research.
In Section \ref{sec:mech}, we sketch the idea of mechanism-based causal inference.
In Section \ref{sec:struct}, we illustrate how the structural modeling approach might be used by accounting researchers and discuss its strengths and weaknesses.
...
Using papers published in 2014 in the \textit{Journal of Accounting Research}, \textit{The Accounting Review}, or \textit{Journal of Accounting and Economics} as a sample, we find that most original research papers use observational data and that about 90\% of these papers seek to draw causal inferences.\footnote{
 ``Original" research excludes papers that are surveys or discussions of other papers.}
While we exclude research papers using experimental methods, all of these papers also seek to draw causal inferences. 
...
However, by stating that ``consistent with \dots theory, $X$ is associated with $Y$," the clear purpose is to argue that the evidence tilts the scale, however slightly, in the director of believing the theory to be a valid description of the real world: in other words, inference.
Since a theory inevitably involves causal relations, such inference is, we argue, inherently causal inference.

To get a sense for the importance of causal questions in accounting research,
we examined all papers published in 2014 in the \textit{Journal of Accounting Research}, \textit{The Accounting Review}, or the \textit{Journal of Accounting and Economics}).
...
The more common approach we observed is the use of estimation approaches such as ordinary least-squares (OLS) regression, difference-in-difference estimates, and propensity-score matching for estimating coefficients that the authors use to draw causal inferences.
While it is widely understood that OLS regressions do not yield unbiased estimates of causal effects, accounting researchers appear incorrectly to view some of these methods as delivering such estimates.\footnote{
...
%TODO: Tighten this discussion of Cadman and Sunder (2014)
Even when assignment to treatment is random, care needs to be taken in interpreting estimates as causal effects.
\citet[p.\,1305]{Cadman:2014cr} conjecture that ``VCs have strong incentives to design compensation schemes that provide CEOs with short-horizon incentives in the fiscal years after the IPO." 
...
\citet{MinuttiMeza:2014fn} points out that ``matching does not necessarily eliminate the endogeneity problem resulting from unobservable variables driving [treatment] and [outcomes]."
...
An important point emphasized those in the structural tradition is that causal reasoning is distinct from statistical reasoning.
%TODO: Relate this point to "structural model" discussion above.
...
In this way, the structural model we put forth embeds our assumptions about what causes what, so $y = x \beta + \epsilon$ can be viewed in the sense used by \citet{Goldberger:1972cq}, as meaning that $x$ causes $y$.
...
While formal definition of the back-door criterion is relatively abstract, its application is relatively straightforward and we use Figure \ref{fig:basic} to illustrate.
...
The point of this discussion is not to question the results of  \citet{Cadman:2014cr}, but to illustrate that the inclusion of controls in regressions is something typically done with very little explanation, and to suggest that greater care regarding inclusion of controls may often be needed.

We argue that researchers should explicitly discuss why they are including variables as controls in regressions.
While the with-and-without-controls approach used by \citet{Larcker:2007aa} has intuitive appeal, a more robust approach would involve careful thinking about the plausible causal relations between the treatment variables, the outcomes of interest, and the potential control variables.
...
\citet{Armstrong:2013io} studies the effect of shareholder voting on future executive compensation.
Because of the plausible existence of unobserved confounding variables that affect both future compensation and shareholder support
claim that their instrument is valid based on reasoning that can be expressed as the  Figure \ref{fig:agl}.
...
\citet[p.\,248]{Pearl:2009vo} provides the following definition of an instrument.
...
One popular source of instruments in accounting, finance, and economics is prior research.
For example, \citet{Balakrishnan:2014js} use the ``exogenous shocks" to analyst coverage in \citet{Kelly:2012ih} as an instrument for changes in voluntary disclosure practices.\footnote{
...
These three boxes, along with the arrows (or lack of arrows) between them are the causal diagram implicit in \citet{Kelly:2012ih}.\footnote{
...
These relations are represented in Figure \ref{fig:bbkl} by the three nodes in the right-most column and the nodes between them.\footnote{
Again \citet{Balakrishnan:2014js} view ``coverage shocks" as exogenous, but it is the shift in analyst coverage driven by brokerage closure that is expected to be exogenous.
The thrust of the analysis below is essentially the same if the ``coverage shocks" are treated as exogenous.} 
These links can be expressed by adding to the causal graph in Figure \ref{fig:bbkl} a node for \textit{Disclosure}, driven by $\textit{Information assymetry}_t$, which is linked to $\textit{Liquidity}_{t+1}$.
...
%TODO: Get citation for Mintzberg, The Natural of Managerial Work.
...
% The value of this descriptive approach to identifying plan features is that
...
% One recent example of insightful descriptive institutional research is Soltes (2014). 
