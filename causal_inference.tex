...
Ultimately we believe that accounting research needs to lean less heavily on statistical models and observational data.
...
\vglue -10pt
\begin{itemize}
\item Minimally there should be an increased emphasis on the study of causal mechanisms. Here we believe that causal diagrams can help clarify what assumptions an accounting researcher must make 
...
\item Increased use of structural models. Structural models provide a more complete characterization of the behavior and institutions that produce a phenomenon of interest. They also allow researchers to reason about what would happen if some features of the model change. With these advantages come disadvantages however. We shall address these later.
	%TODO: Flesh this out somewhat. What are the benefits of doing structural modeling?
\item Because there are many questions in accounting that cannot yet be addressed by formal models, accountant researchers should also promote
descriptive research aimed at developing hypotheses, particularly when it comes to causes and effects. At present, most hypotheses that are tested
...
that empiricists can take to data.
...
A key tool in this approach is a causal diagram or causal graph. These diagrams encode causal assumptions and visually communicate how a causal inference is being drawn from a given research design.
...
A causal diagram consists of nodes and edges. A node represents a random variable and edges connect variables.
Edges in causal diagrams are directed, with an arrow pointing from one node to another. The direction of the arrow communicates the hypothesized causal relation between the two variables.
That arrows have a direction accounts for the ``D" in DAG (Directed Acyclic Graph). The acyclic "A" component means that there must be no cycles in the graph. Cycles cause obvious problems in causal reasoning. An example would be $X \rightarrow Y \rightarrow Z \rightarrow X$. In this graph there is no ultimate cause. Graphs make a distinction between observed and unobserved random variables.
...
At least some of these insights might be used to identify causal mechanisms about information transfer and disclosure.
%
