...
Did International Financial Reporting Standards cause an increase in liquidity in the jurisdictions that adopted it?
...
We believe that causal diagrams can be a useful tool to convey the key elements of a structural model and can also act as an middle-level stand-in when structural modeling of a phenonmenon is in its early stages or is incomplete.\footnote{``Middle-level" here referring to the placement of causal diagrams between relatively informal verbal reasoning and the rigors of a structural model.}
...
Many researchers in accounting appear to view causal inference as a purely statistical exercise.
These is typically very little or no theoretical justification for the validity of variables that are selected as instruments.
It is generally difficult to believe that variables such as firm size, leverage, corporate governance, or director age are viable instrumental variables for typical studies.  Similarly, there are serious concerns about using lagged values of endogenous variables or industry averages of endogenous variables as instruments.\footnote{See \citet{Reiss:2007ej} for a discussion regarding the inability to claim that industry averages are valid instruments.}
...
It is fairly common for accounting researchers to claim that they have tested for the validity of their instrumental variables (actually the exclusion restriction) using some type of statistical test.  This claim sometimes takes the form of our selected instrument is correlated with (i) the endogenous variable and (ii) not correlated with the outcome variable of interest.  It is well-known that such tests are not useful for validating an instrumental variable because it is necessary to show that there is no correlated between the instrument and the unobserved errors in the reduced form model being estimated.  
...
In other words, it is quite possible for completely spurious instruments to deliver bad inferences, yet easily pass tests for weak instruments and tests of overidentifying restrictions. Obviously, causal inferences based on such instrumental variables is completely inappropriate.
...
Thus this new evidence suggests that the instrument of \citet[p.\,912]{Armstrong:2013io} is not credibly valid for the causal effect they seek to estimate.  While the descriptive results are informative for thinking about the impact of shareholder voting, it is difficult for {Armstrong:2013io} to draw a causal inference regarding how shareholder voting affects compensation plan design.
...
We find very no case of an instrumental variable analysis in our survey of accounting research published in 2014 that can withstand rigorous scrutiny.
...
While IV is a classic textbook approach for drawing credible causal inferences, its applicability in actual research settings seems very limited.  
Although a plausible methodological approach, the inability to theoretically justify valid instruments seriously limits the applicability of instrumental variable methods in observational accounting research. 
...
For example, whether an executive compensation plan is approved is a discontinuous function of shareholder support \citet{Armstrong:2013io} and whether a firm had to comply with provisions of Sarbanes-Oxley Act in 2004 \citep{Iliev:2010ic} is a discontinuous function of market float.
...
In the first half of the paper, we have argued that, while causal inference is the goal of most accounting research using observational data, research designs that yield output that can be viewed as unbiased estimates of causal effects using such data likely do not exist for most research settings.
So, do we need to give up on causal inference? We believe that this is far too pessimistic.  There are viable paths forward that do not rely on researchers finding clever and appropriate identification strategies to answer questions of interest.
...
Other fields also seek to draw causal inferences with the problematic features of observational data. 
Despite these concerns, these fields such as epidemiology and medicine have successfully drawn causal inferences from observational data.
In the following, we briefly discuss several case studies of plausible causal inference in these other fields and highlight features that enhanced the credibility of their inferences.
...
This contrasts with the very speculative guesses sometimes used by more contemporary researchers to justify natural experiments.

Another important feature of the case of John Snow and cholera is that widespread acceptance of Snow's hypothesis did not occur until compelling evidence of the mechanism was provided.
...
The Russo-Williamson thesis was arguably also at work in the case of Snow and cholera, where the establishment of a mechanism (i.e., \emph{Vibrio cholerae}) was essential before the causal explanation offered by Snow was widely accepted and also in the case of smoking and lung cancer, which was initially conjectured based on associational evidence, but was widely accepted by 1976.\footnote{
