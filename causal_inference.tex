...
\subsubsection{Difference-in-difference and 4,000 effect estimators}
...
The causal interpretation of fixed-effect estimators is also often unclear.
...
It is easy to see that types $A$ and $C$ will not contribute to the estimate, as these types either always or never have a degree and the estimated effect would be the effect on type $B$. But what does this estimate represent?
It is not the effect of treatment on the treated, which would be some weighted combination of \$4,000 and \$20,000. 
It is not an average treatment effect, as that would need to include the effect on the ``never treated" (type $C$).

While it is not clear that this ``treatment effect for those who wait to get a degree" would be of primary interest.
And interpreting estimates when there are more complex relations between potential outcomes and treatments, as is likely to be the case in realistic research settings, seems even more difficult.

In our view, accounting researchers need to be much more careful using and interpreting fixed-effect estimators. 
In particular, researchers need to clearly demonstrate how their fixed-effect estimates are related to the hypothesis test of interest.
