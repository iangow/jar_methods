...
% However, an alternative approach to empirical research uses theoretical models as the basis for empirical analyses.
...
B & = & b_0 + b_1 \, \mbox{OPTIONS} + X_B\beta\\[.5em]
C_M & = & m_0 + m_1 \, \mbox{SALARY} + X_C\gamma + \xi\\[.5em]
\end{array}
\end{equation}
where OPTIONS is the fraction of a CEO's total pay that is stock-based compensation, 
...
The OPTIONS variable is intended to capture the idea that the more a CEO is rewarded for performance, the
...
\mbox{Pr(Restate)} = \theta_0 \mbox{Pr}\left(\, \theta_1 + \theta_2 \mbox{OPTIONS}
...
Absent a theoretical model of mis- or re-statements, most empirical analyses of these data would summarize them by regressing the restatement indicator on the predictors used in \ref{tab:desc}.
...
Our structural model allows us to say more, particularly when it comes to the signs of the SALARY and OPTIONS variables.
...
Besides the intercepts and OPTIONS coefficients, the only other coefficients that are statistically significant are those on INT and SEG.
...
These two effects explain the negative sign on OPTIONS and the positive sign on SALARY observed in the previous logit results. Thus, this structural model has the potential to rationalize patterns observed in the data.
...
{(p_0-v_0)r_0(b_0 + b_1 \, \mbox{OPTIONS})}
\end{align*}
...
Similarly, $\theta_6$ equals the (scaled) misstatement benefit coefficient on the OPTIONS variable.
Although the descriptive regression coefficients in Table 3 might suggest OPTIONS has a negative effect on misstatements, here, because we model the difference between misstatements and restatements, we find it has the expected positive effect.
...
As can be seen, we are assuming that $p_I$ is independent of OPTIONS. 
But is quite plausible that these investigations are conducted (in part) by a regulator who is strategic much as the auditor in our model, thus giving rise to a link between OPTIONS and $p_I$.
Also we assume that OPTIONS is exogenous, whereas it is plausibly related to the complexity of the business, which may also affect the cost of auditing.
