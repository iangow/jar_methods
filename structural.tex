...
plus some interactions (see Table 3 for a list).
...
Recall that the descriptive logit regression coefficients in Table 2 suggest EQUITY has a negative affect on misstatements.
...
Table 3 contains three different sets of estimates.
...
This can be seen in Table 3 by comparing the "True" coefficients in the last column that were used to generate the data to the other columns.
Because we also used the same data to estimate the logit regression in Table 2, we are able to establish that if we used the wrong (non-strategic) auditor structural model, we would have been led astray in our inferences about the effect of EQUITY.
