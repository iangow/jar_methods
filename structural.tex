...
This assumption is made because in practice we only observe misstatements that were detected and corrected by external monitors after the financial statements were issued. 
...
In our models, we assume that misstatements  are deliberate and made by a single agent, who we refer to as the `CEO.'
...
%\vspace{-3mm}
\begin{enumerate}
\item the manager is indifferent between misstating and not misstating, or:
...
The one sign that does not make sense given the other coefficient estimates is the negative sign on $\theta_5$, however this coefficient is insignificantly different from zero.
...
For illustrative purposes, we can ask what would happen to misstatements and restatements if we  do away with incentive pay and nothing else changes in the model.
...
$$ \dfrac{\mbox{Pr(Restate }\vert \mbox{ No Bonus)}}{\mbox{Pr(Restate }\vert \mbox{ Bonus)}}=\dfrac{\beta^* \times (1-p_A^{**}) \times p_{I}}
...
Often when empirical researchers \emph{do} rely on theoretical papers to motivate hypotheses, but the predictions claimed to be derived from those papers have little obvious connection with the actual content of those papers.
...
These links could at some cost be added to the structural model. 
...
Trying to draw causal inferences from faulty assumptions leading to  an incorrect structural model is not helpful for advancing accounting research.  
...
Despite these challenges, we believe that there is significant value in making the theory underlying empirical research transparent and rigorous.\footnote{The use of structural models in accounting research has been fairly limited to date.  Some recent examples are \citet{Gerakos:2013cl}, \cite{Nikolaev:2014er}, \citet{Zakolyukina:2015aa}, and \citet{Bertomeu:2015aa}.  These three papers model an institutionally rich problem, estimate the derived model, provide estimates for important structural parameters, and also give interesting counterfactuals based on their theoretical models. We view these papers as useful initial steps in applying structural approaches to accounting research questions.}
