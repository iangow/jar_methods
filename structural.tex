...
CEOs on average receive about one million dollars in base pay and their incentive-related pay averages 45\% of their total pay.
...
For illustrative purposes, we evaluate whatwould happen to misstatements and restatements if we do away with equity-based compensation and nothing else changes in the model.
...
From the equilibrium strategies in equation (\ref{equilstrat}), we see that removing equity-based pay ($b1=0$ or EQUITY=0) does not change the equilibrium frequency of misstatements (i.e., $\beta^*$), but does change the frequency of high effort auditing ($\alpha^*$).
...
In our analyses, we in fact generated the data ourselves. That is, the data are not from actual firms, but instead were generate by computer using the strategic auditor model. We chose to do this
