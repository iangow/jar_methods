...
Most empirical studies in accounting rely on observational data. That is, data that
are produced by a process beyond the direct control of the researcher. While there
is nothing wrong with using observational data to describe accounting practices
and institutions, accounting researchers typically step beyond descriptive
conclusions to draw causal inferences.
To highlight the prevalence of this practice, we surveyed papers published in 2014 in the \textit{Journal of Accounting Research}, \textit{The Accounting Review}, or \textit{Journal of Accounting and Economics}) and found that about 90\% of empirical papers doing original research seek to draw causal inferences.\footnote{``Original" research excludes papers that are surveys or discussions of other papers.  We also exclude experimental and field-based research papers, although most of these papers also seek to draw causal inferences. 
...
However, by stating that ``consistent with . . . theory, X is associated with Y," the purpose clearly is to argue that the
evidence tilts the scale, however slightly, in the director of believing the theory is a valid description of the real world: 
in other words, inference. Since a theory is essentially a causal statement, such inference is inherently a causal inference.}

% I'm a little bit reluctant to use the word "exogenous," as no-one knows what it means.
However, the models estimated in these studies generally rely on variations in the key variables of interest that are not exogenous or due to random assignment. 
...
Economists and other social science researchers have recently held out hope that
changes in research designs and statistical methods can increase the credibility
of causal inferences.\footnote{For example, \cite{Angrist:2010jv} claim that ``empirical microeconomics has experienced a credibility revolution, with a consequent increase in policy relevance and scientific impact.''  One especially promising path is use of  field experiments with randomized treatments to measure causal effects (a good example -- Roberts QJE paper)} Perhaps the most active area are the use of quasi-experiments with observational data. These suggestions include
exogenous difference-in-difference and (regression) discontinuity design strategies.along with the traditional instrumental variable strategies to identify causal. Our survey of recent accounting research suggests that these methods have made few appropriate inroads into accounting research, with just ten studies in our survey using instrumental variables and four studies using regression discontinuity designs.  Moreover, even for these specific studies, there are serious questions about the econometric application.
This raises the important issue of whether the theoretical models and econometric methods used in accounting research can produce reasonable causal inferences.
% Dave: I think you're falling into the same trap as Dan was here. Whenever we draw inferences about a theory (model or, more often, a bunch of words) about how the world works, we are doing causal inference. The question is whether those inferences are valid. If all we are doing is reading coefficients off some regression, then we'd better be sure we have some kind of "identification strategy" *and*, as our "structural model" shows, some way of interpreting the coefficients in terms of basic parameters.

At a broad level, this paper seeks to clarify the distinction between descriptive
versus causal statements, and evaluate the extent to which statistical methods can
compensate for the types of observational data used in accounting research. We
begin by describing the current state of observational studies in accounting.
This overview points to several generic problems that observational
data pose for those who wish to draw causal inferences.

We next discuss the use a of graphical or “path” representation of a causal model as a constructive means for thinking broadly about causal issues. While these representations cannot in and of themselves
identify a “correct” statistical or causal model, their use can help researchers visualize what general assumptions must underlie any causal inference. We use the Structural Causal Model \citep{Pearl:2009kh} approach to provide a structure and terminology for the balance of the paper.  The advantage of the SCM approach is that some theoretical question of interest is clearly expressed in erms of exogenous variables (outside the model), endogenous variables (determined within the model), and a set of structural equations linking the endogenous and exogenous variables together.  
...
Our objective is not to dismiss the knowledge gained from prior empirical research, but rather to provide a critical, but informed, assessment of our approach to drawing causal inferences.  
...
\subsection{Causal inference in observational accounting research}
